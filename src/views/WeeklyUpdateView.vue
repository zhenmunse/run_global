...
async function checkFileExists(filename) {
  try {
    const response = await fetch(`/weekly_update/storage/${filename}`, { method: 'HEAD' });
...
async function loadFileContent(filename, force = false) {
  ...
  let url = `/weekly_update/storage/${filename}`;
  const fetchOptions = { method: 'GET', cache: 'no-store' };
  if (force) {
    url = `https://raw.githubusercontent.com/zhenmunse/run_global/main/weekly_update/storage/${filename}?t=${Date.now()}`;
  }
